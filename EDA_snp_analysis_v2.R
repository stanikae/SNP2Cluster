...
# START ANALYSIS ----------------------------------------------------------
  ...
  for(i in seq_along(facility_vec)){
    ...
    kmx <- round(nrow(mat)/2)  # nrow(mat) - 2
    ...
    if(nrow(mat)>10){
      p4 <- factoextra::fviz_nbclust(mat, kmeans , method= 'silhouette',nboot = 500)
    }else{
      p4 <- factoextra::fviz_nbclust(mat, kmeans, k.max = kmx, method= 'silhouette',nboot = 500)
    }
    ...
    if(clust_type == "Core"){
      mst_dir <- file.path(dirname(work_dir),paste0("minimum-spanning-tree","_SNPcutoff",snpco))
    }else{
      mst_dir <- file.path(dirname(work_dir),paste0("minimum-spanning-tree","_SNPcutoff",snpco,"_Days",daysco))
    }
    
    # create outdir
    if (! dir.exists(mst_dir)){
      dir.create(mst_dir,recursive = T)
    }
    
    setwd(mst_dir)
    
    
    # get path for core SNP cluster file
    fileSuffix <- paste("Core-SNP-Clusters",date_var,"xlsx",sep = ".")
    fullFileName <- list.files(work_dir,pattern = fileSuffix, recursive = T)
    cluster_file = file.path(work_dir,fullFileName)
    ...
    if(file.exists(cluster_file)){
      ...
      stopifnot("Core SNP cluster file not found")
    }
    ...
    g <- igraph::graph_from_adjacency_matrix(mst_out, mode="undirected", weighted=T)
